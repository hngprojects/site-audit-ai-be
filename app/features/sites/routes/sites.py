...
async def create_site(
    request: SiteCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
    try:
...
async def get_site(
    site_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
...
async def get_all_sites(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
