        ...
        for section_key, issue_category in categories_map.items():
            ...
            for issue_data in issues:
                try:
                    ...
                    # Extract affected_element from LLM (NEW)
                    affected_element = issue_data.get("affected_element", {})
                    element_selector = affected_element.get("selector") if affected_element else None
                    element_html = affected_element.get("html") if affected_element else None
                    affected_elements_count = 1 if element_selector else 0
                    ...
                    issue = ScanIssue(
                        ...
                        element_selector=element_selector,  # From LLM
                        element_html=element_html,  # From LLM
                        impact_score=impact_score,  # Calculated based on severity
                        affected_elements_count=affected_elements_count,  # From LLM
