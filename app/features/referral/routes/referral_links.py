...
async def generate_referral_link(
    ...
    try:
        ...
        logger.exception(
            f"Unexpected error generating referral link for user {current_user.id}", exc_info=exc
        )
        raise
...
async def track_referral_click(
    ref: str, request_body: TrackClickRequest, request: Request, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        await service.track_click(
            ref_code=ref, source=request_body.source, user_agent=user_agent, ip_address=ip_address
        )

        landing_page_url = settings.LANDING_PAGE_URL.rstrip("/")

        return api_response(
            data={"status": "success", "landingPageUrl": landing_page_url},
...
async def get_referral_stats(ref: str, db: AsyncSession = Depends(get_db)):
    """
