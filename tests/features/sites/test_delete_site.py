...
from app.features.sites.services import site as site_service
...
async def test_delete_site_service_success():
    ...
    mock_site = Site(id=site_id, user_id=user_id, root_url="http://example.com")
...
async def test_delete_site_service_forbidden():
    ...
    mock_site = Site(id=site_id, user_id=other_user_id, root_url="http://example.com")
    mock_result = MagicMock()
    mock_result.scalar_one_or_none.return_value = mock_site
    mock_db.execute.return_value = mock_result
...
@patch("app.features.sites.routes.sites.delete_site")
